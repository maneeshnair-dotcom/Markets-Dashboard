# dashboard_zerodha.py
# ─────────────────────────────────────────────────────────────────────────────
# Zerodha (Kite Connect) FnO Signal Dashboard
# Same dashboard as dashboard.py, but data comes from Zerodha's Kite Connect
# API (via zerodha_analyzer.py) instead of yfinance, and there's a single
# fixed ticker list (FnO) instead of the Nifty/INT/BSE/BANKNIFTY registry.
#
# Requires a Kite Connect API key + secret (from developers.kite.trade) and
# a daily login (Kite access tokens expire every day at ~6am IST) — see the
# "Kite Login" section in the sidebar.
#
# Local run :  streamlit run dashboard_zerodha.py
# ─────────────────────────────────────────────────────────────────────────────
...
# ── Load the Zerodha analyzer ─────────────────────────────────────────────────
ANALYZER_FILE = os.path.join(os.path.dirname(__file__), "zerodha_analyzer.py")

@st.cache_resource(show_spinner=False)
def load_analyzer():
    spec   = importlib.util.spec_from_file_location("zerodha_analyzer", ANALYZER_FILE)
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    return module


# ── Kite Connect credentials & session (adapted from your app(1).py) ──────────
DEFAULT_API_KEY    = ""   # put your Kite API key here, or use st.secrets
DEFAULT_API_SECRET = ""   # put your Kite API secret here, or use st.secrets
...
def get_credentials():
    try:
        api_key = st.secrets.get("KITE_API_KEY", DEFAULT_API_KEY)
    except Exception:
        api_key = DEFAULT_API_KEY
    try:
        api_secret = st.secrets.get("KITE_API_SECRET", DEFAULT_API_SECRET)
    except Exception:
        api_secret = DEFAULT_API_SECRET
    access_token = None
    try:
        if "KITE_ACCESS_TOKEN" in st.secrets:
...
@st.cache_data(ttl=3600, show_spinner="Loading Kite instrument master...")
def get_instruments_cached(_kite):
    return load_analyzer().get_instrument_master(_kite)
...
# ── Single fixed ticker list (FnO) — no multi-list registry ───────────────────
_analyzer_mod   = load_analyzer()
FnO_TICKERS     = _analyzer_mod.FnO
SKYBLUE_TICKERS = getattr(_analyzer_mod, "SKYBLUE_TICKERS", set())
list_name       = "FnO"   # fixed — kept as a plain constant so header/filename code below is unchanged

# ── Market Sentiment basket — EDIT THIS LIST to change the default bellwether
# stocks used for the Market Sentiment badge. You can also add/remove stocks
# live from the sidebar without touching code.
DEFAULT_SENTIMENT_BASKET = ["NIFTY", "SENSEX", "RELIANCE", "HDFCBANK", "BHARTIARTL"]

_ALL_KNOWN_STOCK_NAMES = sorted(FnO_TICKERS)

# ── Kite login (credentials + daily access token) ──────────────────────────────
...
with st.sidebar:
    ...
    if api_key:
        _temp_kite = KiteConnect(api_key=api_key)
        ...
        st.caption("Set KITE_API_KEY / KITE_API_SECRET in Secrets, or DEFAULT_API_KEY/DEFAULT_API_SECRET "
                    "near the top of this file, before logging in.")
    if not access_token:
        access_token = st.text_input("Or paste an access token manually", type="password")
    st.markdown("---")

kite = get_kite_client(api_key, access_token) if (api_key and access_token) else None

if not kite:
    st.title("📈 Zerodha FnO Signal Dashboard")
    st.info("Log in via the sidebar (or paste an access token) to load the dashboard. "
            "Kite access tokens expire daily — you'll need to log in again each trading day.")
    st.stop()

# ── Sidebar controls (these are PENDING choices — see 'applied' below) ────────
with st.sidebar:
    st.markdown("### ⚙️ Settings")
    st.caption(f"Ticker list: **FnO** ({len(FnO_TICKERS)} instruments) — fixed, edit the "
                "`FnO` list in zerodha_analyzer.py to change it.")
    n_days_w   = st.slider("History (days)",    min_value=5,  max_value=90,  value=30,  step=5,
                            help="Kite's historical API enforces its own max lookback per interval "
                                 "(shorter for minute-family intervals). If a fetch fails with a "
                                 "'date range too long' error, reduce this.")
    ...
    sentiment_basket_w = st.multiselect(
        ...
        help="Bellwether stocks used to compute the Market Sentiment badge. "
             "Add or remove any FnO stock here."
    )
    st.markdown("---")
    auto_refresh = st.toggle("AUTO-REFRESH (5 min)", value=False)
    st.caption("⏱ Fetching the full FnO list is slow — Kite's historical API is "
                "rate-limited to ~3 requests/sec per symbol, so a full refresh of "
                f"~{len(FnO_TICKERS)} instruments can take a few minutes.")
    st.markdown("<div style='color:#5D7A99;font-size:12px'>Powered by Zerodha Kite Connect</div>",
                unsafe_allow_html=True)


# ── Data fetching (cached, clears on Refresh) ─────────────────────────────────
@st.cache_data(ttl=300, show_spinner=False)   # 5-min TTL matches auto-refresh
def get_data(_kite, n_days, interval, buy_thr, sell_thr):
    mod = load_analyzer()
    instruments_df = get_instruments_cached(_kite)
    return mod.fetch_fno_data(_kite, n_days, interval, buy_thr, sell_thr,
                               ticker_list=FnO_TICKERS, instruments_df=instruments_df)
...
if "applied_params" not in st.session_state:
    st.session_state.applied_params = dict(
        n_days=n_days_w, interval=interval_w,
...
n_days, interval, buy_thr, sell_thr = (
    applied["n_days"], applied["interval"],
...
with col_title:
    ...
    <div class="dash-header">
      ...
      <div>
        <div class="dash-title">Apex Markets Terminal — FnO (Zerodha)</div>
        <div class="dash-sub">Data via Kite Connect · Composite Signals: RSI · Stochastic · Bollinger · Volume</div>
      </div>
...
with st.spinner(f"⏳  Fetching live Market Data For {list_name} tickers..."):
    try:
        df = get_data(kite, n_days, interval, buy_thr, sell_thr)
        fetch_ok = True
    except Exception as e:
...
if need_reference_tab:
    with st.spinner("⏳  Fetching 1D reference data..."):
        try:
            df_1d = get_data(kite, REF_1D_DAYS, "1d", buy_thr, sell_thr)
        except Exception as e:
